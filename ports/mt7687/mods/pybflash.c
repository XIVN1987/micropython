...
#define FLASH_BASE_ADDR             (0x101F7000 - 1024 * 400)

#define FLASH_BLOCK_SIZE            (1024 * 4)
#define FLASH_BLOCK_COUNT           (1024 * 400 / FLASH_BLOCK_SIZE)
#define FLASH_SECTOR_SIZE           512
#define FLASH_SECTOR_COUNT          (1024 * 400 / FLASH_SECTOR_SIZE)
...
DRESULT flash_disk_flush (void) {
    ...
    if (FLASH_Cache_Dirty) {
        FLASH_PageWrite(FLASH_BASE_ADDR + FLASH_BLOCK_SIZE * FLASH_Block_IndexPre, FLASH_Block_Cache);
...
/******************************************************************************/
// MicroPython bindings to expose the internal flash as an object with the block protocol.

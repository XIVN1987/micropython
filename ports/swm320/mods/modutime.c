...
STATIC mp_obj_t time_mktime(mp_obj_t tuple)
{
    ...
    return mp_obj_new_int_from_uint(
                timeutils_mktime(mp_obj_get_int(elem[0]), mp_obj_get_int(elem[1]), mp_obj_get_int(elem[2]),
...
STATIC mp_obj_t time_sleep(mp_obj_t seconds_in)
{
    uint second = mp_obj_get_int(seconds_in);

    if(second > 0)
    {
        mp_hal_delay_ms(second * 1000);
    }
